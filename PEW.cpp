...
void game() {
  ...
  */
  ...
  while (!GetAsyncKeyState('A')) {
    ...
    if (TabChoix != 0) {
      printc(Couleurs[0], 11);
    } else {
      Beep(1000, 100);
      printc(Couleurs[0], 176);
    }
    if (TabChoix != 1) {
      printc(Couleurs[1], 12);
    } else {
      printc(Couleurs[1], 192);
    }
    if (TabChoix != 2) {
      printc(Couleurs[2], 14);
    } else {
      printc(Couleurs[2], 224);
    }
    if (TabChoix != 3) {
      printc(Couleurs[3], 13);
    } else {
      printc(Couleurs[3], 208);
    }
    if (TabChoix != 4) {
      printc(Couleurs[4], 10);
    } else {
      printc(Couleurs[4], 160);
    }
  }
  switch (TabChoix) {
  case 0:
  ...
  while (JeuEnCours) //Tant que le jeu est en cours:
  {
    ...
    if (PlayGame == 1) //Si on joue au jeu
    {
      ...
      **/
      ...
      textInterface1 = "Let's play! Guess the direction chosen  ";
      textInterface2 = "by Pew. Press (R)IGHT or (L)EFT.        ";
      Interface[12] = "|" + textInterface1 + "|";
      Interface[13] = "|" + textInterface2 + "|";
      if (GetAsyncKeyState('R')) {
        if (ranC == 1) {
          //Gagné
          textInterface1 = "You won!                                ";
          textInterface2 = "                                        ";
          Interface[12] = "|" + textInterface1 + "|";
          Interface[13] = "|" + textInterface2 + "|";
          if (Hap < 36) {
